...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNeosCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNeosCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNeosCoinAmount = strAnonymizeNeosCoinAmount.remove(strAnonymizeNeosCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeosCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNeosCoinAmount*COIN) nMaxToAnonymize = nAnonymizeNeosCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNeosCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNeosCoinAmount));
        strAnonymizeNeosCoinAmount = strAnonymizeNeosCoinAmount.remove(strAnonymizeNeosCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNeosCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
