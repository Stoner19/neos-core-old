...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNeosCoinAmount"))
        settings.setValue("nAnonymizeNeosCoinAmount", 1000);
    nAnonymizeNeosCoinAmount = settings.value("nAnonymizeNeosCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNeosCoinAmount"))
        SoftSetArg("-anonymizetransferamount", settings.value("nAnonymizeNeosCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNeosCoinAmount:
            return QVariant(nAnonymizeNeosCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeNeosCoinAmount:
            nAnonymizeNeosCoinAmount = value.toInt();
            settings.setValue("nAnonymizeNeosCoinAmount", nAnonymizeNeosCoinAmount);
            emit AnonymizeNeosCoinAmountChanged(nAnonymizeNeosCoinAmount);
            break;
        default:
            break;
        }
